...
// Ponto de entrada público — avalia todos os canais candidatos e retorna ranking por banda
fun evaluateChannels(
    neighbors: List<Neighbor>,
    config: EvalConfig = EvalConfig(),
): Map<Band, List<ChannelScore>> {
    // Regra 8 — remove o próprio roteador do cálculo
    ...
    return Band.entries.associateWith { band ->
        ...
        // Regra 7 — ordenação: score → overlappingAps → strongestNeighborDbm → canal
...
private fun scoreChannel(
    ...
    for (ap in neighbors) {
        ...
        for ((apLo, apHi) in spans) {
            // Regra 4 — sobreposição espectral em MHz
            val overlap = overlapMhz(apLo, apHi, cLo, cHi)
            if (overlap <= 0) continue

            val fraction = overlap.toDouble() / width.mhz
            // Regra 3 — acumula no domínio linear (mW), nunca soma dBm diretamente
    ...
    // Regra 6 — penalidade DFS (apenas quando avoidDfs=false; com true os DFS já são excluídos dos candidatos)
