...
// Regra 1 — frequência → (banda, canal de controle)
...
fun channelToCenterFreq(band: Band, channel: Int, width: ChannelWidth): Int = when (band) {
    ...
    Band.GHZ_5 -> alignToGrid(
        ...
        baseFreq = 5170,    // menor frequência válida no padrão (ch 34)
        widthMhz = width.mhz,
    )
...
// Alinha primaryFreq ao centro do segmento W-MHz que o contém, contando a partir de baseFreq
...
// Regra 3 — dBm → potência linear (mW); acumulação sempre no domínio linear
fun dbmToMw(rssiDbm: Int): Double = 10.0.pow(rssiDbm / 10.0)

// Regra 4 — sobreposição espectral em MHz entre dois spans [lo, hi)
