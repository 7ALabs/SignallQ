...
private fun candidates24(allow24Overlapping: Boolean): List<Int> =
    // Regra 5 — apenas 1/6/11 por padrão; allow24Overlapping libera 1–13
    if (allow24Overlapping) (1..13).toList() else listOf(1, 6, 11)

private fun candidates5(width: ChannelWidth, avoidDfs: Boolean): List<Int> {
    // Regra 5 — canais primários padrão por largura (menor canal de cada grupo bondado)
    // Regra 6 — DFS = canais 52–144; excluídos quando avoidDfs=true
...
// Regra 6 — verifica se canal pertence à faixa DFS de 5 GHz (52–144)
fun isDfs5Ghz(channel: Int): Boolean = channel in 52..144

// Regra 5 — verifica se canal é PSC em 6 GHz
