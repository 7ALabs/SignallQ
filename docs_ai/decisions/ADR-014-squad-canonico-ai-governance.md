...
- ~~**Auditar `~/.claude/agents/`** (agentes user-level) e decidir se coexistem com `ai-governance/` (interpretação atual) ou se são substituídos.~~ **Verificado em 2026-08-15:** `~/.claude/agents/` **não existe** nesta máquina (nenhum `.md` presente). A "coexistência" descrita em [DECISAO_CONSOLIDACAO_SQUAD_7ALABS_2026-07-23](DECISAO_CONSOLIDACAO_SQUAD_7ALABS_2026-07-23.md) é conceitual, não empírica neste ambiente. Se o Luiz tem `~/.claude/agents/` em outra máquina, ficam por lá; nada aqui os invoca ou depende deles. Simplifica: só existe uma camada de agentes ativa neste repo, `ai-governance/agents/`.
- ~~**Mover skills que citam Lia/Felipe/Gema como responsáveis** ... Fora do escopo desta rodada — cada skill deve ser corrigida oportunisticamente quando tocada, conforme regra de higiene (seção 8).~~ **Resolvido em 2026-08-15 (PR #1615):** 7 skills alinhadas em um passo (Lia → Juliana, Rhodolfo → Caio). Nenhuma menção legada sobrou em `.claude/skills/`, `.claude/commands/` ou `.claude/fluxos/`.
- ~~**Remover fisicamente** `scripts/agent-handoff.sh`, `notify.sh`, `discord_notify.sh` após confirmar que nada em CI/hooks os invoca.~~ **Resolvido em 2026-08-15 (PR #1615):** movidos para `scripts/legacy/` (não removidos — git preserva, mas fora do fluxo ativo). Ver [`scripts/legacy/README.md`](../../../scripts/legacy/README.md).

## Pendências estruturais (para decisão do Luiz, issues abertas)

- **#1616 — Reduzir contrato operacional a 1 página + skills executáveis.** O contrato atual (133 linhas) foi lido uma vez e ignorou-se por meses; a drift que motivou este ADR é evidência. Proposta: 1 página de invioláveis + `/check-done` e `/handoff` como skills obrigatórias em transição.
- **#1617 — Instrumentar métricas de handoff.** Sem latência de handoff, rework do gate único, throughput e WIP visíveis, as decisões de squad continuam anedóticas. Proposta: Worker + D1 + página no Admin, revisada semanalmente.
- **`.claude/settings.local.json`** ainda contém paths Windows (`C:\Users\luizg\...`) — tentei limpar em 2026-08-15 mas o classifier de auto-mode bloqueou reescrita de arquivo de settings. Requer edição manual do Luiz ou permission rule para desbloquear.
