...
| `.../DiagnosticOrchestrator.kt` | Fachada `StateFlow` do diagnóstico; delega para `NdsDiagnosticRepository.evaluate` quando `consumer_diagnostico_nds_live_enabled` está ligada (NDS-02k/#1759), senão para `RemoteDiagnosticRepository.evaluateShadow` (default hoje — flag desligada em todo ambiente) |
