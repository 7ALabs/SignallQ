...
| `UptimeChartUseCase` | `feature/history/.../UptimeChartUseCase.kt` | Classifica latência alta como `"OFFLINE"` — **rótulo enganoso**, não é ausência de conectividade |
...
| `UptimeChartUseCase`/`UptimeGridChart` (feature/history) | **Não** | Thresholds próprios de latência (300/800ms) inteiramente desconexos de `MetricClassifier`, com bucket de "offline" que na verdade significa "latência alta" |
...
| Latência | `UptimeChartUseCase.kt:30-31` | `<=300 OK`, `<=800 LENTO`, `>800 OFFLINE` | ms | rótulo "OFFLINE" p/ latência alta | tela Histórico/Uptime | Sim | **Sim — rótulo enganoso, P0** |
