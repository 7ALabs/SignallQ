...
- **Atualização 2026-07-31 (Fatia 9a):** **P1-1 corrigido** — ver seção "Fatia 9" abaixo. `feature/diagnostico.RecommendationEngine` (REC-01..REC-14) renomeado para `RecomendacaoPraticaEngine`, eliminando a colisão de nome com `core/recommendation.RecommendationEngine` (motor de monetização). Rename puro, comportamento de recomendação inalterado. **P1-2 registrado como issue de acompanhamento #1528, decisão de produto.**
- **Atualização 2026-08-01 (P1-2 resolvido):** `RecommendationRequestMapper.mapTags()` agora consome `report.recomendacoes` em vez de re-derivar por regex, alinhando-se ao comportamento do `RecomendacaoPraticaEngine`. As 6 tags com regra REC equivalente (REC-01/02/04/05/06/10/11) leem direto do motor legado; 2 tags sem equivalente (MUITOS_DISPOSITIVOS, LATENCIA_ALTA) preservam régex sobre IDs brutos. Remova `CONTRATADO_TOLERANCIA` (divergência com REC-04). Testes validam caso crítico: causa externa suprime recomendação como esperado. Commit [51218cd8](https://github.com/buildea-labs/SignallQ/commit/51218cd8).
- **Atualização 2026-07-31 (rodada de fechamento):** confirmado o merge de 4 PRs que fecham achados desta auditoria — [#1523](https://github.com/buildea-labs/SignallQ/pull/1523) (Fatia 5, P0-2), [#1525](https://github.com/buildea-labs/SignallQ/pull/1525) (Fatia 8, P0-8), [#1526](https://github.com/buildea-labs/SignallQ/pull/1526) (Fatia 6, P1-4) e [#1527](https://github.com/buildea-labs/SignallQ/pull/1527) (Fatia 9a, P1-1). `main` em `538c257c` após o merge das quatro. Ver Parte 8 para o detalhe por achado.
...
| ID | Achado | Evidência |
|---|---|---|
| **P1-1** | ~~Dois engines chamados literalmente `RecommendationEngine` (`core/recommendation` e `feature/diagnostico`), sem relação de chamada, tipos de entrada/saída diferentes, com colisão real de nome no autocomplete/import.~~ **RESOLVIDO (2026-07-31, Fatia 9a, PR [#1527](https://github.com/buildea-labs/SignallQ/pull/1527))** — engine legado (`feature/diagnostico`, REC-01..REC-14) renomeado para `RecomendacaoPraticaEngine`. Rename puro, sem alteração de comportamento (`recomendar()` idêntico, testes existentes intactos). | `core/recommendation/.../RecommendationEngine.kt` (monetização, mantido), `feature/diagnostico/.../RecomendacaoPraticaEngine.kt` (renomeado) |
| **P1-2** | ~~`RecommendationRequestMapper.mapTags()` deriva tags para o engine #1 independentemente das regras REC-01..14 do engine #2 (regex de prefixo de ID em vez de ler o resultado), incluindo um cálculo de "% do plano contratado" com tolerância diferente (20%) do que REC-04 usa (`linkSpeed`, não `download`).~~ **RESOLVIDO (2026-08-01, issue [#1528](https://github.com/buildea-labs/SignallQ/issues/1528), commit [51218cd8](https://github.com/buildea-labs/SignallQ/commit/51218cd8))** — `mapTags()` agora consome `report.recomendacoes` (saída real do `RecomendacaoPraticaEngine`) para as 6 tags com regra REC-01..14 dedicada (REC-01/02→WIFI_FRACO, REC-04→VELOCIDADE_ABAIXO_DO_CONTRATADO, REC-05→BUFFERBLOAT_ALTO, REC-06→DNS_LENTO, REC-10→SINAL_BAIXO, REC-11→PERDA_PACOTES_ALTA), elimina a constante `CONTRATADO_TOLERANCIA` que divergia do `linkSpeed` usado por REC-04, preserva régex sobre IDs brutos para tags sem equivalente REC (MUITOS_DISPOSITIVOS, LATENCIA_ALTA). Testes novos validam comportamento crítico: causa externa (DNS lento) suprime WIFI_FRACO como esperado pelo motor legado. | `RecommendationRequestMapper.kt:77-136` |
...
- **Como foi implementada de fato:** decisão tomada direto (sem esperar Fatia 1/2 formais) —
  ...
  [#1523](https://github.com/buildea-labs/SignallQ/pull/1523) (branch `fix/1228-fatia5-rssi-wifi-unificado`).
...
- **Como foi implementada de fato:** `core/diagnostico.MetricClassifier.classificarBufferbloat` já
  ...
  PR desta fatia: [#1526](https://github.com/buildea-labs/SignallQ/pull/1526) (branch `fix/1228-fatia6-bufferbloat-consolidado`).
...
- **Como foi implementada de fato (Fatia 9a) — CONCLUÍDA (2026-07-31):** só a metade de baixo risco
  ...
  **A metade de risco médio (P1-2, mudar a lógica de `RecommendationRequestMapper.mapTags()` para
  ...
  atualização da linha P1-2 na Parte 5 e a issue [#1528](https://github.com/buildea-labs/SignallQ/issues/1528), aberta na mesma fatia.
  PR desta fatia: [#1527](https://github.com/buildea-labs/SignallQ/pull/1527).
