...
export default function ProPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col overflow-x-hidden" style={{ background: 'var(--bg-primary)' }}>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/40 p-5" onClick={() => setModalOpen(false)}>
          <div
            onClick={(e) => e.stopPropagation()}
            className="flex w-full max-w-[380px] flex-col items-center gap-3.5 rounded-3xl p-7 text-center"
            style={{ background: 'var(--bg-card)' }}
          >
            <span className="material-symbols-outlined" style={{ fontSize: 34, color: '#0B6CFF' }}>
              workspace_premium
            </span>
            <div className="title-large">O SignallQ PRO estará disponível em breve</div>
            {!interesseRegistrado ? (
              <>
                <div className="body-medium">Deixe seu e-mail e avisamos assim que o PRO lançar.</div>
                <input
                  type="email"
                  value={email}
              ...
              <div className="body-medium" style={{ color: 'var(--success)' }}>
                Anotado! Você será avisado assim que o SignallQ PRO lançar.
              </div>
            )}
            <div className="mt-1.5 flex w-full gap-2.5">
              <button onClick={() => setModalOpen(false)} className="h-11 flex-1 rounded-[var(--radius-button)] border label-large" style={{ borderColor: 'var(--border)' }}>
                Fechar
              </button>
              <button onClick={() => navigate('/')} className="h-11 flex-1 rounded-[var(--radius-button)] text-white label-large" style={{ background: 'var(--accent)' }}>
                Usar o SignallQ gratuito
              </button>
            </div>
          </div>
