...
export function StepRow({ steps }: { steps: StepInfo[] }) {
  return (
    <div className="flex w-full max-w-[680px] overflow-hidden rounded-3xl border glass-panel sq-fade-up" style={{ borderColor: 'color-mix(in srgb, var(--border) 22%, transparent)' }}>
      {steps.map((step, i) => (
        <div
          key={step.label}
