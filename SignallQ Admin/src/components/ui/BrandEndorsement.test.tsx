...
describe("BrandEndorsement", () => {
  ...
  it("resolve o símbolo oficial da Buildea quando variant=symbol-text, sem exigir symbolSrc", () => {
    ...
    expect(img).toHaveAttribute("src", expect.stringContaining("brand/buildea/symbol.png"));
  });

  it("usa o mesmo símbolo independente de theme (já vem com fundo próprio embutido)", () => {
    const { container: darkContainer } = render(<BrandEndorsement variant="symbol-text" theme="dark" />);
    const { container: lightContainer } = render(<BrandEndorsement variant="symbol-text" theme="light" />);
    expect(darkContainer.querySelector("img")).toHaveAttribute("src", expect.stringContaining("brand/buildea/symbol.png"));
    expect(lightContainer.querySelector("img")).toHaveAttribute("src", expect.stringContaining("brand/buildea/symbol.png"));
  });
