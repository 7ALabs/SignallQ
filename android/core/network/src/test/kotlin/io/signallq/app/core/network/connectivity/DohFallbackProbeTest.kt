...
class DohFallbackProbeTest {
    ...
    private fun respostaDnsBinaria(rcode: Int, ancount: Int): ByteArray {
    ...
    @Test
    fun `sucesso quando o provedor DoH responde NOERROR com pelo menos um registro`() = runBlocking {
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
    ...
    @Test
    fun `timeout quando o provedor DoH nao responde dentro do prazo`() = runBlocking {
        server.enqueue(
            MockResponse()
    ...
    @Test
    fun `falha de resolucao quando o provedor DoH responde NXDOMAIN`() = runBlocking {
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
