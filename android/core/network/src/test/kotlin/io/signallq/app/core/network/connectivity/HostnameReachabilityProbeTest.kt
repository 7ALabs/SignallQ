...
class HostnameReachabilityProbeTest {
    ...
    @Test(timeout = 6_000L)
    fun `sondagem de hostname cujo connect trava retorna dentro de um prazo real`() =
        runBlocking {
            val probe =
                HostnameReachabilityProbe(
    ...
    @Test(timeout = 6_000L)
    fun `cancelamento durante o connect nao acumula custo da proxima url`() =
        runBlocking {
            val probe =
                HostnameReachabilityProbe(
                    binding = BindingComConnectTravado(),
