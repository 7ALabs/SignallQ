...
class DiagnosticRolloutStatusRepositoryTest {
    ...
    @Test
    fun `current mapeia o payload completo do worker`() = runTest {
        server.enqueue(
    ...
    @Test
    fun `current mapeia rulesetVersion null quando nao ha ruleset publicado`() = runTest {
        server.enqueue(
            MockResponse().setResponseCode(200).setBody(
    ...
    @Test
    fun `current devolve null quando o worker responde erro e nao ha cache anterior`() = runTest {
    ...
    @Test
    fun `current usa cache dentro do TTL, sem nova chamada de rede`() = runTest {
        server.enqueue(
            MockResponse().setResponseCode(200).setBody(
    ...
    @Test
    fun `current busca de novo apos o TTL expirar`() = runTest {
        server.enqueue(
            MockResponse().setResponseCode(200).setBody(
                """{"rulesetVersion":1,"rolloutPercent":50,"rolloutMinVersionCode":null,"rolloutChannels":null}""",
            ),
    ...
    @Test
    fun `current mantem o ultimo valor cacheado quando uma busca posterior falha`() = runTest {
        server.enqueue(
            MockResponse().setResponseCode(200).setBody(
