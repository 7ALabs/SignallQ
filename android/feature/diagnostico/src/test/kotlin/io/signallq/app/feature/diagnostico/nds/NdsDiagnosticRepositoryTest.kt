...
class NdsDiagnosticRepositoryTest {
    ...
    @Test
    fun `NDS respondendo com sucesso - relatorio vem REMOTE com o veredicto do NDS`() =
        runTest {
    ...
    @Test
    fun `Assist com NDS respondendo com sucesso - relatorio vem REMOTE`() =
        runTest {
    ...
    @Test
    fun `NDS respondendo com sucesso - dispara telemetria de outcome success`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody(successBody()))
    ...
    @Test
    fun `NDS respondendo com sucesso - dispara telemetria de cobertura do snapshot`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody(successBody()))
    ...
    @Test
    fun `NDS respondendo 401 - telemetria de cobertura aponta bloco critico ausente quando aplicavel`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(401)
    ...
    @Test
    fun `NDS retorna regular sem causa nem recomendacao - preserva resposta remota`() =
        runTest {
    ...
    @Test
    fun `NDS respondendo 401 (KnownError) - cai para motor local sem excecao`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(401)
    ...
    @Test
    fun `Assist com NDS respondendo 401 - propaga erro sem fallback local`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(401)
    ...
    @Test
    fun `NDS respondendo 401 - dispara telemetria known_error com fallback usado`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(401)
    ...
    @Test
    fun `NDS fora do ar (timeout) - cai para motor local sem travar`() =
        runTest {
    ...
    @Test
    fun `Assist com NDS fora do ar - propaga erro explicito sem fallback local`() =
        runTest {
            server.enqueue(MockResponse().setSocketPolicy(SocketPolicy.DISCONNECT_AT_START))
    ...
    @Test
    fun `NDS respondendo 500 com corpo nao-JSON (UnknownError) - cai para motor local`() =
        runTest {
    ...
    @Test
    fun `NDS respondendo 200 com corpo malformado - NdsClient ja devolve UnknownError, repository cai para motor local`() =
        runTest {
    ...
    @Test
    fun `perfilGamer true - request enviado ao NDS carrega profile gamer`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody(successBody()))
    ...
    @Test
    fun `perfilGamer default false - request enviado ao NDS nao carrega profile`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody(successBody()))
    ...
    @Test
    fun `Assist com usarNdsV2=false - chama v1 identico ao comportamento anterior`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody(successBody()))
    ...
    @Test
    fun `Assist com usarNdsV2=true sem subcategory na origem chama v2`() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(200).setBody("""{"raw":{},"explanation":{"titulo":"t","descricao":"d","dados":[]}}"""))
            val inputComObjective =
                snapshotSaudavelInput().copy(
    ...
    @Test
    fun `Assist com usarNdsV2=true - resposta v2 e mapeada para REMOTE com explanation`() =
        runTest {
            ...
            val outcome =
                clientQueChamaV2.evaluate(
                    io.signallq.app.core.nds.NdsDiagnosticsRequest(
                        requestId = "exec-nds-v2-test",
                        app =
                            io.signallq.app.core.nds
                                .NdsAppInfo(id = "io.signallq.app", version = "1.0.0"),
                        context =
                            io.signallq.app.core.nds.NdsDiagnosticContext(
    ...
    @Test
    fun `Assist com usarNdsV2=true e NDS falhando - continua sem fallback local (comportamento existente)`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(401)
