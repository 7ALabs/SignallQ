...
class TopologyDiagnostic(
    ...
    suspend fun diagnose(): NetworkTopology =
        coroutineScope {
            ...
            val vendor =
                gatewayIp?.let { ouiLookup.lookup(it) }
                    ?: upnpInfo?.manufacturer?.let { ouiLookup.lookup(it) }

            val router =
                if (gatewayIp != null || upnpInfo != null) {
                    DeviceInfo(
                        ip = gatewayIp,
                        mac = null,
                        vendor = vendor,
                        ...
                        model =
                            listOfNotNull(upnpInfo?.modelName, upnpInfo?.modelNumber)
                                .joinToString(" ")
