...
private class FakeMedicaoDao(
    ...
    override suspend fun atualizarDiagnostico(
        id: String,
        texto: String?,
        origem: String?,
        problemas: String?,
    ...
    override suspend fun atualizarScore(
        id: String,
        score: Double,
    ) = Unit
...
class UptimeChartUseCaseTest {
    @Test
    fun `retorna exatamente 336 blocos para 7 dias`() =
        runBlocking {
    ...
    @Test
    fun `blocos sem medicoes sao classificados como SEM_DADO`() =
        runBlocking {
            val useCase = UptimeChartUseCase(FakeMedicaoDao(emptyList()))
            val blocos = useCase.gerar7dias()
            assertTrue(
    ...
    @Test
    fun `medicoes com latencia baixa classificam bloco como OK`() =
        runBlocking {
    ...
    @Test
    fun `medicoes com latencia alta classificam bloco como LENTO`() =
        runBlocking {
            val agora = System.currentTimeMillis()
            val medicoes =
                listOf(
    ...
    @Test
    fun `medicoes com latencia muito alta classificam bloco como LATENCIA_ALTA, nunca OFFLINE`() =
        runBlocking {
    ...
    @Test
    fun `medicoes sem latencia mensuravel classificam bloco como OFFLINE`() =
        runBlocking {
            val agora = System.currentTimeMillis()
            val medicoes =
                listOf(
    ...
    @Test
    fun `blocos sao ordenados do mais antigo para o mais recente`() =
        runBlocking {
            val useCase = UptimeChartUseCase(FakeMedicaoDao(emptyList()))
            val blocos = useCase.gerar7dias()
